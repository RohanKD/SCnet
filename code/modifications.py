...
if __name__ == '__main__':
    ...
    bdata = anndata.read_h5ad("../data/Mouse_wholebrain_FC.h5ad")
    cdata = anndata.read_h5ad("../data/Mousecortex_protocols.h5ad")
    combined = adata[adata.obs['cell.type'] == "Neuron"]
    valid = ["Neuron", "Interneuron", "Astrocytes"] #["Neuron", "Interneuron", "Astrocytes", "Oligodendrocytes", "Polydendrocytes", "Microglia", "Endothelial"]
    for i in valid:
        combined = anndata.AnnData.concatenate(*[combined, adata[adata.obs['cell.type'] == i]],
                                               join="inner")
    adata = combined
    combined = bdata[bdata.obs['cell.type'] == "Neuron"]

    for i in valid:
        combined = anndata.AnnData.concatenate(*[combined, bdata[bdata.obs['cell.type'] == i]],
                                               join="inner")
    bdata = combined
    combined = cdata[cdata.obs['cell.type'] == "Neuron"]

    for i in valid:
        combined = anndata.AnnData.concatenate(*[combined, cdata[cdata.obs['cell.type'] == i]],
                                               join="inner")
    cdata = combined
    samples = []
